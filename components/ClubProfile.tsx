        ...
        {displayPosts.length > 0 ? (
          <div className="columns-1 md:columns-2 gap-4 space-y-4">
            {displayPosts.map(post => (
              <div key={post.id} className="break-inside-avoid mb-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col">
                 ...
                 <div className="px-4 py-3 border-t border-gray-50 dark:border-gray-700 flex items-center justify-between mt-auto">
                   <div className="flex items-center gap-4">
                      ...
                      <button className="flex items-center gap-1.5 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors">
  ...
  return (
    <div className="animate-fade-in pb-20">
      ...
      <div className="relative h-48 bg-gray-200 dark:bg-gray-700">
        <img src={`https://picsum.photos/seed/${club.id}/800/300`} className="w-full h-full object-cover" alt="Cover" />
        <button
          onClick={onBack}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 border border-gray-100 dark:border-gray-700">
          <div className="flex justify-between items-start">
            <div className="flex gap-4">
              <img src={club.logo} alt={club.name} className="w-20 h-20 rounded-xl border-4 border-white dark:border-gray-800 shadow-sm object-cover bg-gray-100 dark:bg-gray-700" />
              <div className="pt-10 sm:pt-0">
                <h1 className="text-xl font-bold text-gray-900 dark:text-white leading-tight mb-1">{club.name}</h1>
                <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 gap-3">
                  <span className="flex items-center gap-1"><Users size={14} /> {club.membersCount} 成員</span>
                  <span className="flex items-center gap-1 text-yellow-500"><Star size={14} fill="currentColor" /> {club.rating}</span>
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              {managedClubIds.includes(club.id) ? (
                <>
                  ...
                  <button className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-4 py-2 rounded-full text-sm font-bold border border-gray-200 dark:border-gray-600">
                    管理社團
                  </button>
                </>
